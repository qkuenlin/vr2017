...
public class KinectModelPosition : MonoBehaviour {
	...
	void Update () {
		...
		// set model position
		if (updatePosition) {
			CameraSpacePoint pos = data[bodyNbr].Joints[JointType.SpineBase].Position;
			...
			if (data[bodyNbr].Joints[JointType.SpineBase].TrackingState == TrackingState.Tracked)
